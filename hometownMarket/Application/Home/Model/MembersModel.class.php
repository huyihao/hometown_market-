...
public function login($user_name,$user_pwd){
		...
		$user_info=D('Members')->where("members_username='{$user_name}' AND members_password='{$user_pwd}' AND members_visitset='0'")->find();
// 		dump($this->getLastSql());
		if($user_info){
			...
			D('Members')->where("members_username='{$user_name}'")->setField('members_lastlogindate',date());
			D('Members')->where("members_username='{$user_name}'")->setField('members_lastloginip',$_SERVER['REMOTE_ADDR']);
			
			return $info;
		}else{
			$info=array(
					'status'=>0,
					'info'	=>'登陆失败,用户没有访问权限或者账户密码不正确!',
			);
			return $info;
		}
